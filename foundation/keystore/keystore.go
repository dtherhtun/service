// Package keystore is an in-memory keystore for JWT support.
package keystore
...
func NewFS(fsys fs.FS) (*KeyStore, error) {
	...
	fn := func(fileName string, dirEntry fs.DirEntry, err error) error {
		if err != nil {
			return fmt.Errorf("walkdir failture: %w", err)
		}
		...
		if err != nil {
			return fmt.Errorf("parsing auth key file: %w", err)
		}
...
// Remove removes a private key and combination kid from the store.
